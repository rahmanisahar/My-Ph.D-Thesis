...
\cite{Geach12} and~\cite{Rahmani16b} demonstrate the algorithm of the SOM in more detail.
     ...
     There have been a few attempts to find a rule for restricting the number of neurons based on the input sample \citep[e.g.][]{Vesanto05}, but none of them are certain. 
...
%  Fig. ~\ref{fig: sample_SOMZ} shows the SOM of this mock sample. 
%  The upper panel is the distance map. 
%  The axes show the position of the neurons in the $10 \times 10$ network.
%  The lower panel in Fig.~\ref{fig: sample_SOMZ} shows the hit map.
...
% The upper panel in the Fig.~\ref{fig: sample_SOMZ} clearly shows this division.
...
% The lower panel in Fig.~\ref{fig: sample_SOMZ} shows that only 4 neurons (out of 100) are occupied. 
   ...
   Based on the size of the data and SOM results, we found the optimum grid size to be $1\times22$ and $12\times12$ in 1D and 2D maps, respectively. 
    ...
    \label{sec: 1D_somz}
        ...
        \label{sec: 1Dt}
            ...
            As in to Fig.~\ref{fig: sample_SOMZ}, the upper part of the figures shows the neurons and their relative distances between weight of neurons.
