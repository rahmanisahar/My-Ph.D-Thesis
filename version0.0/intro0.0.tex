...
In order to determine the stellar populations of spiral galaxies, the mean stellar mass density of a galaxy may be a more basic parameter than the total stellar mass~\citep{Bell00}. 
\cite{Kauffmann03} tested that on more than 100,000 galaxies observed by the Sloan Digital Sky Survey~\citep[SDSS;][]{York00}. %PB: what is "that"?
They also showed that these results applied to all morphological types. %PB: what are "these results" ?
...
The first is measuring the dynamical mass of a galaxy using kinematics~\citep{Cappellari06} or lensing~\citep{Auger09}, and then modelling the mass of the dark matter and subtracting it from the measured mass. %PB: "kinematics" and "lensing" are astro-jargon: please explain a bit more
This method has been successful to predict and subtract the mass of the dark matter, which is the dominant mass in galaxies~\citep{Zaritsky94}, and can easily cause uncertainties as high as the order of the measurement. %PB: but is it successful in measuring the stellar mass?

The second method is based on stellar population models~\citep[e.g.][]{ Bruzual93, Kotulla09} and using them to connect stellar mass to an observable, such as luminosity in different wavelengths, colours, or spectral energy distribution. %PB: need a one-sentence description of what these models do.
This method has uncertainties too, and one must also model the uncertainties. %PB: sentence not useful.
Uncertainties come from uncertainties in the stellar initial mass function (see Sec.~\ref{sec: imf}), the stellar evolution models, especially the stage where stars have the most luminosity in their evolution, the evolutionary state of the galaxy~\citep[see,][]{Dalcanton12}, and star formation histories. 

%PB: this paragraph was very difficult to understand and I did a lot of rewriting. Please check.
In order to translate between photometry and dynamics the stellar mass-to-light ratio ($M/L$) is needed. %PB: is this the first method or the second one? It's not really about dynamics.
...
Having two independent methods to measure stellar mass helps to check whether there are any systematic differences. 
Comparing the results from measuring stellar mass in stellar clusters shows there is no huge difference. %PB: ref?
Comparing the results from measuring stellar mass within galaxies shows that the differences are on the order of a few x10 per cent, so one should be careful to model subtleties~\citep{McLaughlin05}. 

\subsection{Dynamical Mass} %PB: suggest "galaxy masses from dynamics"
% PB: should following paragraph go after the second paragraph?
...
Thanks to the current generation of detectors, it is possible to obtain  very high-resolution spectra in the \halpha and CO lines  over most of the optical disks of galaxies. % and cover most regions in the sky. PB: not sure about "most regions of sky"
Integrated line widths only yield an estimate of total mass within some (uncertain) isophotal radius.

Resolved rotation curves produced from \halpha, \hi and CO lines are proportional to optical emission lines within the optical disk of galaxies,~\citep{Courteau13} which helps us to measure the rotational curves. %PB: don't get the purpose of this sentence
The simplest way to estimate the mass of a galaxy is assuming that the physics of the galaxy is simple Newtonian physics. %PB should be clear that this is what nearly everyone does (almost no one assumes that physics isn't Newtonian)
...
\end{equation}
where $\Sigma_0$ the central surface brightness, $R_d$ the disk scale length, $4y=\frac{R}{2R_d}$, and $I_i(y)$ and $K_i(y)$ are the first and second kinds  modified Bessel functions. %PB what is y? Also need to define scale length.
...
%There are many different profiles to fit the rotation curve for galaxies. PB: not useful.
A detailed review of dynamical masses of galaxies can be found in \citet{Courteau13}.

\subsection{Stellar Population Method} %PB: suggest  "galaxy masses from stellar population modelling"
...
However, some fraction of the mass of galaxies consists of evolved stars which no longer emit radiation yet still contribute to the galaxy mass as stellar remnants. %PB: so what do we do about this?
...
the age, $t$, of the population, measured in years, also referred to as the time passed since the star formation began;
the star formation history (SFH), often parametrized with SFR $\propto e^{-1/\tau}$; the IMF; the metallicity; and the chemical composition ratios, usually measured relative to values measured in the Sun. 
Stellar evolutionary tracks, the stellar spectral libraries, and the parametrization for mass-loss which affects several late stages of evolution are necessary for stars placing at specific evolutionary phases~\citep{Courteau13}. %PB: this sentence has a lot of astro-jargon

%PB: please think about how info in following few paragraphs should logically go together into paragraphs.
...
The calculated stellar mass can be used to calibrate the fluxes from the stars as the easily measurable parameters of stars. %PB: don't understand what "as the easily.." means here.
Using NIR bands, different groups tried to make a map of stellar mass distribution within galaxies~\citep[e.g.,][]{Elmgreen84}.
Using $B-V$ colours~\citep{Bell01} and {\em Spitzer} 3.6- and 4.5-$\mu$m, M/L correction, \cite{Kendall08} produced stellar mass surface densities of M81. %PB: unclear how this  sentence relates to other text.

\cite{Zibetti08} suggested that to reach the maximum spatial resolution one can recombine the photometric information on a pixel-by-pixel basis~\citep{Conti03}. 
They used this method to study the stellar mass distribution in galaxies as well as the dependence of the SED on stellar mass density. %PB: join paragraphs. 
...
\end{equation}
where $\Sigma \lambda(\alpha, \delta)$ is the surface brightness and $\gamma_{\lambda}(\alpha, \delta)$ is the effective stellar M/L at an effective wavelength $\lambda$. % PB what are \alpha, \delta ?
...
They used 3.6 and 4.5$\mu$m fluxes for calibration of the stellar mass, because these two bandpasses are almost insensitive to the emission from young stellar population or to dust absorption and emission. 
Thanks to the \Spitzer~\citep{Wener04} and WISE~\citep{Wright10} telescopes, a large amount of data in these bands from many regions in the sky is available, which helps thave a more accurate calibration. 
...
\end{equation}
where M$_{\star}$ is the stellar mass and $L = \nu L_{\nu}[\lambda]$ is luminosity in erg~s$^{-1}$/$\AA$. %PB: units of stellar mass? 
 
\cite{Zhu10} used the SDSS and the {\em Spitzer} Wide-area Infrared Extragalactic Survey~\citep[SWIRE;][] {Lonsdale03} to find another calibration between galaxy luminosity and stellar mass. 
...
Measuring the stellar mass from equation~\ref{equ:eskew} is limited to nearby galaxies, and might break down for highly star-forming regions; moreover, this equation has not tested on regions with different metallicity. 
Thus there is no information on the effects of variation in  metallicity~\citep{Eskew12}. 
On the other hand equations~\ref{equ: zhu1}--~\ref{equ: zhu3} have been derived from a large sample of galaxies, and were tested on different type of regions and galaxies. 
They can be used for more distant galaxies as well as nearby galaxies~\citep{Zhu10}. %PB: why is Zhu et al good for distant galaxies but Eskew is not?
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%Preview%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
A complete picture of how galaxies are formed and evolve is tied to unfolding the mystery of the formation of stars.
 Various phenomena affect the collapse of molecular clouds and star formation, including the environment of the star-forming region, chemical composition of the region, the initial density distribution of the gas, the mass of sounding stars and other physical properties of galaxies. %PB "sounding stars" ? "other physical properties" is vague - either be specific or delete it.
 For decades, observational, theoretical and numerical studies have investigated star formation; nevertheless, their results have never been precise. %PB: not really clear what you mean by "precise" here.
...
%For each chapter, we also describe the methodology and data used in that chapter. %PB: not needed.
