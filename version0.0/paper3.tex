...
as there are few studies on relations between properties of M31 with its PAH features. %generate?!
...
Those authors re-evaluated the ISOCAM data to compare them to IRS data and found that the earlier results were likely due to incorrect background subtraction. %(PB), but the results were inconclusive. $$Sr:?
...
We describe the SOM method in $\S$~\ref{sec: method}. 
...
\begin{figure}
  \begin{subfigure}[b]{\textwidth}
    ...
    \end{subfigure}
    \caption{Position of the data in M31 (a) and M101 (b).}
\end{figure}
...
%Why 1D maps are useful
   ...
   \label{fig: M31_nets_1d}
    ...
    \subsection{Inside the 2 neuron network}
        ...
        \caption[Pearson correlation coefficients for data from clustered regions in M31]{Same as Fig.~\ref{fig: cor_all}, but here we used data from regions grouped in the left sides of Figs.~\ref{fig: M31_net_1by2} and ~\ref{fig: M31_net_1by3}. }
       ...
       % Some studies have indicated that strong stellar emission could lead to destruction of PAHs ~\citep[e.g.][]{Clayton03,Seok14}. %%% These are all UV emission
...
%----------------------------------------------------------------------------------------
 ...
 \label{sec: 2d_cluster}
    ...
    \label{sec: subsets}
            ...
            These result is consistent with results from~\cite{Draine14}, who showed that the amount of PAHs in M31 for regions within 20~kpc radius. %PB: part of sentence missing?
            ...
            Although with changing input parameters for each network the  distances between neurons and positions of the regions change, the general positions of the regions  relative to one another does not change dramatically. %PB: changed "data" to "regions" in this sentence - please check.
    ...
    %This process is time efficient %PB:sentence incomplete?
...
The method described in this work can be used to select target regions for PAH observations with that facility. %%Eh % better now?
